...
if (isset($_SESSION['id_admin'])) {
    ...
    if (isset($_POST['submit'])) {
        ...
        $query      = mysqli_query($koneksi, "INSERT INTO `tabel_soal` VALUES ('$kategori','$pertanyaan','$pilihan_a','$pilihan_b','$pilihan_c','$pilihan_d','$jawaban','$publish','$tipe')");

        if ($query) {
            ?>
            	<script language="javascript" type="text/javascript">document.location.href="?page=view";</script>
            <?php
        } else {
            echo mysqli_error();
        }
	...
	<form action="?page=soal" method="post">
		...
		<div class="form-group">
			<textarea class="form-control" rows="3" name="pertanyaan"
				placeholder="Pertanyaan" cols=""></textarea>
		</div>
		...
		<div class="form-group">
			<label>Jawaban</label> <select name="jawaban"
				class="btn btn-default dropdown-toggle">
				<option value="a">A</option>
				<option value="b">B</option>
				<option value="c">C</option>
				<option value="d">D</option>
			</select> <label>Publish</label> <select name="publish"
				class="btn btn-default dropdown-toggle">
				<option value="yes">Yes</option>
				<option value="no">No</option>
			</select> <label>Tipe</label> <select name="tipe"
				class="btn btn-default dropdown-toggle">
